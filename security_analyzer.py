...
REMEDIATION_ADVICE = {
    ...
    "WP_CONFIG_BAK_EXPOSED": {
        "default": "Supprimez immédiatement le fichier de sauvegarde de configuration WordPress exposé publiquement."
    },
    "WP_USER_ENUM_ENABLED": {
        "default": "Empêchez l'énumération des utilisateurs sur WordPress, par exemple en utilisant un plugin de sécurité ou en ajoutant des règles de réécriture."
    },
}
...
def scan_tls_protocols(hostname, verbose=False):
    ...
    try:
        ...
        for result in scanner.get_results():
            if result.scan_status == ServerScanStatusEnum.ERROR_NO_CONNECTIVITY:
                return [
                    {
                        "statut": "ERROR",
                        "message": f"Impossible de se connecter à {hostname} pour le scan TLS.",
                        "criticite": "HIGH",
                    }
                ]
            proto_scans = {
            ...
            for name, scan in proto_scans.items():
                if scan.status == ScanCommandAttemptStatusEnum.ERROR:
                    continue
                if scan.result.accepted_cipher_suites:
                    crit = (
                        "HIGH"
                        if name in ["SSL 2.0", "SSL 3.0", "TLS 1.0", "TLS 1.1"]
                        else "INFO"
                    )
                    res = {
                        "protocole": name,
                        "statut": "ERROR" if crit == "HIGH" else "SUCCESS",
                        "message": "Supporté",
                        "criticite": crit,
                    }
                    if crit == "HIGH":
        ...
        return [
            {
                "statut": "ERROR",
                "message": f"Erreur inattendue lors du scan sslyze: {e}",
                "criticite": "HIGH",
            }
        ]
...
def check_http_to_https_redirect(hostname, verbose=False):
    ...
    try:
        ...
        return {
            "statut": "ERROR",
            "message": "La redirection de HTTP vers HTTPS n'est pas correctement configurée.",
            "criticite": "MEDIUM",
            "remediation_id": "NO_HTTPS_REDIRECT",
        }
    except Exception as e:
        return {
            "statut": "ERROR",
            "message": f"Erreur lors du test de redirection: {e}",
            "criticite": "HIGH",
        }
...
def check_dns_records(hostname, verbose=False):
    ...
    try:
        ...
        results["ns"] = {
            "statut": "SUCCESS",
            "valeurs": [str(r.target) for r in ns_ans],
            "criticite": "INFO",
        }
    except Exception as e:
        results["ns"] = {
            "statut": "ERROR",
            "message": f"Impossible de récupérer les enregistrements NS ({e})",
            "criticite": "LOW",
        }
    try:
        ...
        results["a"] = {
            "statut": "SUCCESS",
            "valeurs": [r.address for r in a_ans],
            "criticite": "INFO",
        }
    except Exception as e:
        results["a"] = {
            "statut": "ERROR",
            "message": f"Impossible de récupérer les enregistrements A ({e})",
            "criticite": "LOW",
        }
    try:
        ...
        results["mx"] = {
            "statut": "SUCCESS",
            "valeurs": [f"Prio {p}: {e}" for p, e in mx_records],
            "criticite": "INFO",
        }
    except Exception as e:
        results["mx"] = {
            "statut": "ERROR",
            "message": f"Impossible de récupérer les enregistrements MX ({e})",
            "criticite": "LOW",
        }
    try:
    ...
    try:
        ...
        spf_rec = next(
            (
                s
                for s in [" ".join([b.decode() for r in txt_ans]) for r in txt_ans]
    ...
    try:
        ...
        if not raw_cookies:
            return [
                {
                    "statut": "INFO",
                    "message": "Aucun cookie n'a été défini par le serveur.",
                    "criticite": "INFO",
                }
            ]
        for header in raw_cookies:
            ...
            cookie_res["secure"] = {
                "present": secure_ok,
                "criticite": "INFO" if secure_ok else "HIGH",
                "remediation_id": "COOKIE_NO_SECURE",
            }
            cookie_res["httponly"] = {
                "present": httponly_ok,
                "criticite": "INFO" if httponly_ok else "MEDIUM",
                "remediation_id": "COOKIE_NO_HTTPONLY",
            }
            cookie_res["samesite"] = {
                "present": samesite_ok,
                "criticite": "INFO" if samesite_ok else "MEDIUM",
                "remediation_id": "COOKIE_NO_SAMESITE",
            }
        ...
        return [
            {
                "statut": "ERROR",
                "message": f"Erreur lors de la récupération des cookies: {e}",
                "criticite": "HIGH",
            }
        ]
    ...
    try:
                ...
                results["empreinte"].append(
                    {
                        ...
                        "valeur": headers[h],
                        "criticite": "LOW",
                        "remediation_id": "SERVER_HEADER_VISIBLE",
                    }
                )
        hsts_header = headers.get("strict-transport-security")
        if (
            hsts_header
            and "max-age" in hsts_header
            and int(hsts_header.split("max-age=")[1].split(";")[0]) >= 15552000
        ):
            results["en-tetes_securite"]["hsts"] = {
                "statut": "SUCCESS",
                "criticite": "INFO",
            }
        else:
        ...
        if xfo_header in ["DENY", "SAMEORIGIN"]:
            ...
            results["en-tetes_securite"]["x-frame-options"] = {
                "statut": "ERROR",
                "criticite": "MEDIUM",
                "remediation_id": "XFO_MISSING",
            }
        xcto_header = headers.get("x-content-type-options", "").lower()
        if xcto_header == "nosniff":
            results["en-tetes_securite"]["x-content-type-options"] = {
                "statut": "SUCCESS",
                "criticite": "INFO",
            }
        else:
            results["en-tetes_securite"]["x-content-type-options"] = {
                "statut": "ERROR",
                "criticite": "MEDIUM",
                "remediation_id": "XCTO_MISSING",
            }
        csp_header = headers.get("content-security-policy")
        if csp_header:
            results["en-tetes_securite"]["csp"] = {
                "statut": "SUCCESS",
                "criticite": "INFO",
            }
        else:
            results["en-tetes_securite"]["csp"] = {
                "statut": "WARNING",
                "criticite": "LOW",
                "remediation_id": "CSP_MISSING",
            }
        return results
    except Exception as e:
        return {
            "statut": "ERROR",
            "message": f"Erreur lors de la récupération des en-têtes: {e}",
            "criticite": "HIGH",
        }
...
def check_cms_footprint(hostname, verbose=False):
    ...
    try:
        ...
        if gen_tag and gen_tag.get("content"):
            return {
                "statut": "INFO",
                "message": f"Balise 'generator' trouvée: {gen_tag.get('content')}",
                "criticite": "INFO",
            }
        return {
            "statut": "INFO",
            "message": "Aucune balise meta 'generator' trouvée.",
            "criticite": "INFO",
        }
    except Exception as e:
        return {
            "statut": "ERROR",
            "message": f"Erreur lors de l'analyse CMS: {e}",
            "criticite": "HIGH",
        }
...
def check_js_libraries(hostname, verbose=False):
    ...
    try:
        ...
        for script in soup.find_all("script", src=True):
            ...
            match = re.search(
                r"([a-zA-Z0-9.-]+?)[._-]([0-9]+\.[0-9]+(?:\.[0-9]+)?)(?:[._-]min)?\.js",
                src,
            )
            if match:
        ...
        for script in soup.find_all("script"):
            ...
            if "jquery" not in detected_libs and (
                "jQuery" in content or re.search(r"\$\s*\(", content)
            ):
                detected_libs["jquery"] = {
                    "version": "inconnu",
            ...
            if detected_version_str == "inconnu":
                result_entry.update(
                    {
                        "statut": "WARNING",
                        "criticite": "LOW",
                        "message": "Bibliothèque détectée mais version inconnue.",
                    }
                )
            else:
                ...
                try:
                    ...
                    if detected_v < latest_v:
                        ...
                        vulns = query_osv_api(
                            lib_name, detected_version_str, lib_info["ecosystem"]
                        )
                        if vulns:
                            ...
                            for v in vulns:
                                result_entry["vulnerabilities"].append(
                                    {
                                        "id": v.get("id"),
                                        "summary": v.get("summary", "Pas de résumé."),
        ...
        return [
            {
                "statut": "ERROR",
                "message": f"Erreur lors de l'analyse des bibliothèques JS: {e}",
                "criticite": "HIGH",
            }
        ]
    return results
...
def check_wordpress_specifics(hostname, verbose=False):
    ...
    try:
        url = f"{base_url}/wp-config.php.bak"
        response = requests.head(url, timeout=5, allow_redirects=False)
        if response.status_code == 200:
            results["config_backup"] = {
                "statut": "ERROR",
                "criticite": "CRITICAL",
                "message": f"Le fichier de sauvegarde {url} est exposé publiquement.",
                "remediation_id": "WP_CONFIG_BAK_EXPOSED",
            }
        else:
            results["config_backup"] = {
                "statut": "SUCCESS",
                "criticite": "INFO",
                "message": "Le fichier wp-config.php.bak n'a pas été trouvé.",
            }
    except requests.exceptions.RequestException:
        results["config_backup"] = {
            "statut": "INFO",
            "criticite": "INFO",
            "message": "Erreur réseau lors de la vérification de wp-config.php.bak.",
        }
    try:
        ...
        if 300 <= response.status_code < 400 and "/author/" in location:
            username = location.split("/author/")[1].split("/")[0]
            results["user_enum"] = {
                "statut": "ERROR",
                "criticite": "MEDIUM",
                "message": f"L'énumération d'utilisateurs est possible. Nom d'utilisateur trouvé : '{username}'.",
                "remediation_id": "WP_USER_ENUM_ENABLED",
            }
        else:
            results["user_enum"] = {
                "statut": "SUCCESS",
                "criticite": "INFO",
                "message": "L'énumération d'utilisateurs via ?author=1 ne semble pas possible.",
            }
    except requests.exceptions.RequestException:
        results["user_enum"] = {
            "statut": "INFO",
            "criticite": "INFO",
            "message": "Erreur réseau lors de la vérification de l'énumération d'utilisateurs.",
        }
    try:
        ...
        found_plugins = set()
        for tag in soup.find_all(["link", "script"], href=True) + soup.find_all(
            "script", src=True
        ):
            ...
            if url:
                ...
                if match:
                    found_plugins.add(match.group(1))
        if found_plugins:
            results["plugin_enum"] = {
                "statut": "INFO",
                "criticite": "INFO",
                "message": "Plugins détectés",
                "plugins": list(found_plugins),
            }
        else:
            results["plugin_enum"] = {
                "statut": "INFO",
                "criticite": "INFO",
                "message": "Aucun plugin détecté depuis la page d'accueil.",
            }
    except requests.exceptions.RequestException:
        results["plugin_enum"] = {
            "statut": "INFO",
            "criticite": "INFO",
            "message": "Erreur réseau lors de l'énumération des plugins.",
        }
    return results
...
def _format_whois_value(value):
    ...
    if isinstance(value, list):
        ...
        return ", ".join(
            [v.isoformat() if isinstance(v, datetime) else str(v) for v in value]
        )
    ...
    try:
        ...
        return {
            ...
            "registrant_address": ", ".join(
                filter(lambda x: x and x != "N/A", registrant_address)
            )
            or "N/A",
        }
    except Exception as e:
    ...
    if ssl_cert:
        ...
        if jours is not None:
            desc += (
                f" (dans {jours} jours)"
                if jours >= 0
                else f" (expiré depuis {-jours} jours)"
            )
        rows.append(
            {
    ...
    def flatten_data(category, sub_category, data):
        if isinstance(data, list):
            ...
            if "statut" in data and data["statut"] in ["ERROR", "WARNING"]:
                vuln_ids = (
                    ", ".join([v["id"] for v in data["vulnerabilities"]])
                ...
                rows.append(
                    {
                        "Catégorie": category,
                        "Sous-catégorie": data.get("protocole")
                        or data.get("nom")
                        or data.get("bibliotheque")
                        or sub_category,
                        "Statut": data.get("statut"),
                        "Criticité": data.get("criticite"),
                        "Description": data.get("message")
                        or f"Version: {data.get('version_detectee')} (Dernière: {data.get('derniere_version')})"
                        or "Détail non disponible",
                        "Vulnérabilités": vuln_ids,
                    }
                )
                if sub_category == "plugin_enum" and data.get("plugins"):
                    rows[-1][
                        "Description"
                    ] = f"Plugins détectés: {', '.join(data['plugins'])}"
            ...
            if "wordpress_specifics" in data:
                for k, v in data["wordpress_specifics"].items():
                    flatten_data(category, k, v)
            if "secure" in data:
                if not data["secure"]["present"]:
                    flatten_data(category, f"{data['nom']} - secure", data["secure"])
                if not data["httponly"]["present"]:
                    flatten_data(
                        category, f"{data['nom']} - httponly", data["httponly"]
                    )
                if not data["samesite"]["present"]:
                    flatten_data(
                        category, f"{data['nom']} - samesite", data["samesite"]
                    )

    for key, res in results.items():
        if key in ["hostname", "score_final", "note", "ssl_certificate", "whois_info"]:
            continue
...
def generate_html_report(results, hostname):
    ...
    for category, data in results.items():
        if category in ["hostname", "score_final", "note"]:
            continue
        html_content += (
            f"<div class='report-card'><h2>{category.replace('_', ' ').title()}</h2>"
        )
        ...
        if category == "ssl_certificate":
            ...
            if data.get("statut") == "SUCCESS":
                ...
                html_content += (
                    f"<strong>Sujet</strong>: {data.get('sujet', 'N/A')}<br>"
                )
                html_content += (
                    f"<strong>Émetteur</strong>: {data.get('emetteur', 'N/A')}<br>"
                )
            ...
            if data.get("statut") == "SUCCESS":
                ...
                html_content += (
                    f"<strong>Registrar:</strong> {data.get('registrar', 'N/A')}<br>"
                )
                ...
                html_content += (
                    f"<strong>Statut:</strong> {data.get('domain_status', 'N/A')}<br>"
                )
            ...
            for item in data:
                ...
                if item.get("vulnerabilities"):
                    ...
                    for vuln in item["vulnerabilities"]:
                        html_content += f"<li>{vuln['id']}: {vuln['summary']}</li>"
                    html_content += "</ul>"
            ...
            for key, sub_data in data.items():
                if not isinstance(sub_data, dict) or "criticite" not in sub_data:
                    continue
                ...
                if key == "plugin_enum" and sub_data.get("plugins"):
                    html_content += "<ul>"
                    for plugin in sub_data["plugins"]:
                        html_content += f"<li>{plugin}</li>"
                    html_content += "</ul>"
...
def print_human_readable_report(results, verbose=False):
    ...
    print(
        f"  {icon} {crit_str(ssl_cert.get('criticite'))} {ssl_cert.get('message', 'Aucune donnée.')}"
    )
    ...
    for proto in results.get("tls_protocols", []):
        ...
        print(
            f"  {icon} {crit_str(proto.get('criticite'))} {proto.get('protocole', '')} : {proto.get('message', '')}"
        )
    ...
    print(
        f"  {icon} {crit_str(redirect.get('criticite'))} {redirect.get('message', 'Aucune donnée.')}"
    )
    ...
    if headers.get("statut") == "ERROR":
        ...
        for fp in headers.get("empreinte", []):
            print(
                f"    {STATUS_ICONS['INFO']} {crit_str(fp.get('criticite'))} {fp.get('header', '').title()} : {fp.get('valeur')}"
            )
        ...
        for name, data in headers.get("en-tetes_securite", {}).items():
            ...
            print(
                f"    {icon} {crit_str(data.get('criticite'))} {name.replace('_', '-').title()} : {message}"
            )
    ...
    if not cookies or cookies[0].get("criticite") == "INFO":
        print(
            f"  {STATUS_ICONS['INFO']} {cookies[0].get('message') if cookies else 'Aucune information sur les cookies.'}"
        )
    else:
        for cookie in cookies:
            ...
            for name in ["secure", "httponly", "samesite"]:
                ...
                print(
                    f"    {icon} {crit_str(attr.get('criticite'))} {name.title()} : {'Présent' if attr.get('present') else 'Manquant'}"
                )
    ...
    for key, title in dns_order:
        ...
        if data.get("statut") == "SUCCESS":
            ...
            print(
                f"    {icon} {crit_str(data.get('criticite'))} {data.get('message', 'Erreur inconnue.')}"
            )
        print_remediation(data)
    ...
    if not js_libs:
        print(
            f"  {STATUS_ICONS['INFO']} Aucune bibliothèque JS connue n'a été détectée."
        )
    else:
        for lib in js_libs:
            ...
            if "bibliotheque" in lib:
                ...
                if lib.get("statut") == "WARNING":
                    message += (
                        f" - Obsolète (Dernière version: {lib['derniere_version']})"
                    )
    ...
    if "wordpress_specifics" in results:
        print("\n--- Analyse spécifique à WordPress ---")
        wp_results = results["wordpress_specifics"]
        for key, data in wp_results.items():
            if not data:
                continue
            icon = STATUS_ICONS.get(data.get("statut"), "❓")
            print(
                f"  {icon} {crit_str(data.get('criticite'))} {data.get('message', '')}"
            )
            if "plugins" in data:
    ...
    if "whois_info" in results:
        ...
        if whois_info.get("statut") == "SUCCESS":
            ...
            print(
                f"    Nom du propriétaire  : {whois_info.get('registrant_name', 'N/A')}"
            )
            print(
                f"    Organisation         : {whois_info.get('registrant_org', 'N/A')}"
            )
            print(
                f"    Adresse              : {whois_info.get('registrant_address', 'N/A')}"
            )
            print(
                f"    Date de création     : {whois_info.get('creation_date', 'N/A')}"
            )
            print(
                f"    Date d'expiration    : {whois_info.get('expiration_date', 'N/A')}"
            )
            print(f"    Dernière mise à jour : {whois_info.get('updated_date', 'N/A')}")
            print(
                f"    Statut du domaine    : {whois_info.get('domain_status', 'N/A')}"
            )
...
def main():
    ...
    if not check_host_exists(hostname):
        print(
            f"Erreur : L'hôte '{hostname}' est introuvable. Veuillez vérifier le nom de domaine."
        )
        sys.exit(1)
