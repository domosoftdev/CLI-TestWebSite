...
def load_scan_results(verbose=False):
    ...
    for filename in scan_files:
        try:
            ...
            print(
                f"Avertissement : Impossible de parser le fichier '{filename}'. Erreur: {e}"
            )
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Outil de consolidation pour les rapports de sécurité."
    )
    parser.add_argument(
        "--list-scans",
        metavar="DOMAIN",
        help="Liste tous les scans disponibles pour un domaine, triés par date.",
    )
    parser.add_argument(
        "--compare",
        nargs=3,
        metavar=("DOMAIN", "DATE1", "DATE2"),
        help="Compare les scans d'un domaine entre deux dates (format YYYY-MM-DD).",
    )
    parser.add_argument(
        "--quick-wins",
        metavar="DOMAIN",
        nargs="?",
        const="all",
        help="Identifie les vulnérabilités 'quick win' pour un domaine spécifique ou pour tous les domaines.",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="Affiche l'état des scans par rapport à une liste de cibles.",
    )
    parser.add_argument(
        "--oldest", action="store_true", help="Affiche les scans les plus anciens."
    )
    parser.add_argument(
        "--list-expiring-certs",
        nargs="?",
        ...
        type=int,
        metavar="DAYS",
        help="Liste les certificats expirant bientôt (par défaut: 30 jours).",
    )
    parser.add_argument(
        "--report",
        nargs="+",
        metavar="TYPE",
        help="Génère un rapport d'actions pour un ou plusieurs types de vulnérabilités (ex: dmarc, hsts, ou 'all').",
    )
    parser.add_argument(
        "--summary-html",
        action="store_true",
        help="Génère un rapport de synthèse HTML pour tous les sites cibles.",
    )
    parser.add_argument(
        "--graph",
        metavar="DOMAIN",
    ...
    if not os.path.exists(SCAN_REPORTS_DIR):
        print(
            f"Le répertoire des scans '{SCAN_REPORTS_DIR}' n'existe pas. Veuillez le créer et y placer vos rapports JSON."
        )
        return
...
def compare_scans(all_scans, domain, date1_str, date2_str, verbose=False):
    ...
    scan2 = next(
        (s for s in all_scans if s["domain"] == domain and s["date"].date() == d2), None
    )
...
def display_oldest_scans(all_scans, verbose=False):
    ...
    for target in targets:
        most_recent_scan = next(
            (
                s
                for s in sorted(all_scans, key=lambda x: x["date"], reverse=True)
                if s["domain"] == target
            ),
            None,
        )
...
def _get_quick_wins(scan_data):
    ...
    return {
        v for v in vulns if any(rem_id in v for rem_id in QUICK_WIN_REMEDIATION_IDS)
    }
...
def display_quick_wins(all_scans, domain_filter, verbose=False):
    ...
    for domain in target_domains:
        ...
        if quick_wins:
            found_any = True
            print(
                f"--- {domain} (Scan du {most_recent_scan['date'].strftime('%Y-%m-%d')}) ---"
            )
...
def display_expiring_certificates(all_scans, days_threshold, verbose=False):
    ...
    for cert in expiring_certs:
        ...
        print(
            f"  - {cert['domain'].ljust(30)} Expire le: {date_str} (dans {days} jour{plural_s})"
        )
...
def generate_vulnerability_report(all_scans, report_types, verbose=False):
    ...
    if "all" in [rt.lower() for rt in report_types]:
        ...
        for rt in report_types:
            if rt.lower() in SUPPORTED_REPORTS:
                ...
                print(
                    f"Avertissement : Le type de rapport '{rt}' n'est pas supporté. Les types supportés sont : {', '.join(SUPPORTED_REPORTS.keys())}"
                )
    ...
    for report_type, affected_domains in results.items():
        remediation_id = SUPPORTED_REPORTS[report_type]
        advice = REMEDIATION_ADVICE.get(remediation_id, {}).get(
            "default", "Aucun conseil de remédiation disponible."
        )
    ...
    if not found_any_issue:
        print(
            "🎉 Félicitations ! Aucun des problèmes recherchés n'a été trouvé sur les derniers scans de vos domaines."
        )
...
def generate_html_summary(all_scans, verbose=False):
    ...
    try:
        ...
        print(
            "Le fichier 'targets.txt' est introuvable. Veuillez le créer pour générer le rapport de synthèse."
        )
        return
    ...
    <body>
        ...
        <p class="footer">Généré le """
        + today.strftime("%d %B %Y à %H:%M:%S")
        + """</p>
        <table>
            <thead>
                <tr>
...
def generate_evolution_graph(all_scans, domain, verbose=False):
    ...
    if len(scans_for_domain) < 2:
        print(
            f"Moins de deux scans trouvés pour '{domain}'. Impossible de générer un graphique d'évolution."
        )
        return
